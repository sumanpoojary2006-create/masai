...
-- Run the full schema first, then the migration below it.
-- ============================================================
...
CREATE OR REPLACE FUNCTION get_my_batch_role()
RETURNS TEXT AS $$
...
CREATE POLICY "batches_insert" ON public.batches
  FOR INSERT WITH CHECK (get_my_batch_role() IN ('admin', 'editor'));

CREATE POLICY "batches_update" ON public.batches
  FOR UPDATE USING (get_my_batch_role() IN ('admin', 'editor'));

CREATE POLICY "batches_delete" ON public.batches
  FOR DELETE USING (get_my_batch_role() = 'admin');
...
CREATE POLICY "user_roles_admin_all" ON public.user_roles
  FOR ALL USING (get_my_batch_role() = 'admin');

-- ============================================================
-- Auto-create viewer role on first sign-up
-- ============================================================

CREATE OR REPLACE FUNCTION handle_new_batch_user()
RETURNS TRIGGER AS $$
BEGIN
  -- Block signups from non-masaischool.com emails
  IF NEW.email NOT LIKE '%@masaischool.com' THEN
    RAISE EXCEPTION 'Only @masaischool.com email addresses are permitted.';
  END IF;
...
DROP TRIGGER IF EXISTS on_auth_user_created_batch ON auth.users;
CREATE TRIGGER on_auth_user_created_batch
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE PROCEDURE handle_new_batch_user();

-- ============================================================
-- Set your first admin user manually after sign-up:
--
--   UPDATE public.user_roles
--   SET role = 'admin'
--   WHERE user_id = (SELECT id FROM auth.users WHERE email = 'your@masaischool.com');
--
-- ============================================================
