...
export function CCManagementTab() {
  ...
  const filteredBatches = unassigned.filter(
    (b) =>
  ...
  const totalBatches = assignments.length + unassigned.length;
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-white/8 bg-[#10162a] p-6">
          <div className="mb-4 flex items-center justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-400">
                Pull latest batches and sync lecture compliance data.
              </p>
            </div>
          ...
          <p className="text-xs text-slate-500">
            {totalBatches} batch{totalBatches !== 1 ? "es" : ""} in cache · {unassigned.length}{" "}
            unassigned · auto-syncs every Sunday at 2 AM IST
          </p>
        </section>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div ref={batchSearchRef} className="relative flex-1 min-w-[200px]">
              <input
                ...
                placeholder={
                  selectedBatch
                    ? `${selectedBatch.name}${selectedBatch.program ? ` · ${selectedBatch.program}` : ""}`
                    : "— Select Unassigned Batch —"
                }
                value={batchSearch}
                onChange={(e) => {
                ...
                <ul className="absolute z-20 mt-1 max-h-56 w-full overflow-auto rounded-lg border border-white/10 bg-[#1a2236] py-1 shadow-xl">
                  {filteredBatches.map((b) => (
                    ...
                    >
                      {b.name}
                      {b.program ? (
                        <span className="ml-1 text-slate-400">· {b.program}</span>
                      ) : null}
                    </li>
                  ))}
                </ul>
